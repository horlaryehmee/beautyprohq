...
const gateways = [
    { id: 'paystack', name: 'Paystack', description: 'Enter your Paystack subaccount code for booking settlement.' },
...
export default function ProviderPaymentsPage() {
    ...
    return <div className="space-y-6">
        ...
        {activeGateway && <div className="fixed inset-0 z-[70] grid place-items-end bg-slate-950/35 backdrop-blur-sm sm:place-items-center sm:p-4" onMouseDown={() => setActiveGateway(null)}><Card className="w-full max-w-lg rounded-b-none sm:rounded-3xl" onMouseDown={(event) => event.stopPropagation()}><h2 className="text-lg font-bold text-slate-950">Connect {activeGateway.name}</h2><p className="mt-1 text-sm text-slate-500">Save only the destination account reference. Platform secret keys stay in admin settings.</p><form className="mt-5 space-y-4" onSubmit={saveAccount}><Field label="Account name"><input className={inputClass} onChange={(event) => setAccount((current) => ({ ...current, account_name: event.target.value }))} required value={account.account_name} /></Field><Field hint={activeGateway.id === 'paystack' ? 'Example: ACCT_xxxxxx or your Paystack subaccount code.' : 'Example: acct_xxxxxx from Stripe Connect.'} label={activeGateway.id === 'paystack' ? 'Paystack subaccount code' : 'Stripe connected account ID'}><input className={inputClass} onChange={(event) => setAccount((current) => ({ ...current, account_identifier: event.target.value }))} required value={account.account_identifier} /></Field><Field hint="Optional publishable/public key only. Never paste a secret key here." label="Public key"><input className={inputClass} onChange={(event) => setAccount((current) => ({ ...current, public_key: event.target.value }))} value={account.public_key} /></Field><label className="flex items-center gap-3 text-sm font-semibold text-slate-700"><input checked={account.enabled} className="size-4 accent-fuchsia-600" onChange={(event) => setAccount((current) => ({ ...current, enabled: event.target.checked }))} type="checkbox" />Accept booking payments through this gateway</label><div className="flex justify-end gap-2"><Button onClick={() => setActiveGateway(null)} type="button" variant="secondary">Cancel</Button><Button busy={saving} type="submit">Save connection</Button></div></form></Card></div>}
    </div>;
}
