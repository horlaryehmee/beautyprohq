...
const gateways = [
    ...
    { id: 'stripe', name: 'Stripe', description: 'Enter your Stripe Connect account ID for booking settlement.' },
];
...
export default function ProviderPaymentsPage() {
    ...
    const [account, setAccount] = useState({ account_name: '', account_identifier: '', public_key: '', secret_key: '', enabled: true });
    ...
    useEffect(() => {
        ...
        setAccount({ account_name: saved?.account_name ?? '', account_identifier: saved?.account_identifier ?? saved?.account_reference ?? saved?.email ?? '', public_key: saved?.public_key ?? '', secret_key: '', enabled: saved?.enabled ?? saved?.is_connected ?? true });
    ...
    const saveAccount = async (event) => {
        ...
        try {
            const updated = await apiRequest('put', '/provider/payment-accounts', { gateway: activeGateway.id, ...account, settings: account.secret_key ? { secret_key: account.secret_key } : undefined });
    ...
    return <div className="space-y-6">
        ...
        {activeGateway && <div className="fixed inset-0 z-[70] grid place-items-end bg-slate-950/35 backdrop-blur-sm sm:place-items-center sm:p-4" onMouseDown={() => setActiveGateway(null)}><Card className="w-full max-w-lg rounded-b-none sm:rounded-3xl" onMouseDown={(event) => event.stopPropagation()}><h2 className="text-lg font-bold text-slate-950">Connect {activeGateway.name}</h2><p className="mt-1 text-sm text-slate-500">{activeGateway.id === 'paystack' ? 'Use the public and secret keys from your own Paystack dashboard. Booking payments will be initialized and verified on that provider account.' : 'Save the destination account reference. Platform secret keys stay in admin settings.'}</p><form className="mt-5 space-y-4" onSubmit={saveAccount}><Field label="Account name"><input className={inputClass} onChange={(event) => setAccount((current) => ({ ...current, account_name: event.target.value }))} required value={account.account_name} /></Field>{activeGateway.id === 'paystack' ? <><Field label="Paystack public key"><input className={inputClass} onChange={(event) => setAccount((current) => ({ ...current, public_key: event.target.value }))} placeholder="pk_test_..." required value={account.public_key} /></Field><Field hint="Encrypted after saving. Leave blank when editing to keep the saved key." label="Paystack secret key"><input className={inputClass} onChange={(event) => setAccount((current) => ({ ...current, secret_key: event.target.value }))} placeholder={accounts?.paystack?.has_secret_key ? 'Saved — leave blank to keep current key' : 'sk_test_...'} type="password" value={account.secret_key} /></Field></> : <><Field hint="Example: acct_xxxxxx from Stripe Connect." label="Stripe connected account ID"><input className={inputClass} onChange={(event) => setAccount((current) => ({ ...current, account_identifier: event.target.value }))} required value={account.account_identifier} /></Field><Field hint="Optional publishable/public key only." label="Public key"><input className={inputClass} onChange={(event) => setAccount((current) => ({ ...current, public_key: event.target.value }))} value={account.public_key} /></Field></>}<label className="flex items-center gap-3 text-sm font-semibold text-slate-700"><input checked={account.enabled} className="size-4 accent-fuchsia-600" onChange={(event) => setAccount((current) => ({ ...current, enabled: event.target.checked }))} type="checkbox" />Accept booking payments through this gateway</label><div className="flex justify-end gap-2"><Button onClick={() => setActiveGateway(null)} type="button" variant="secondary">Cancel</Button><Button busy={saving} type="submit">Save connection</Button></div></form></Card></div>}
    </div>;
}
