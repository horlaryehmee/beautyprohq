...
export default function ProviderPaymentsPage() {
    ...
    const [account, setAccount] = useState({ account_name: '', account_identifier: '', public_key: '', secret_key: '', mode: 'sandbox', enabled: true });
    ...
    useEffect(() => {
        ...
        setAccount({ account_name: saved?.account_name ?? '', account_identifier: saved?.account_identifier ?? saved?.account_reference ?? saved?.email ?? '', public_key: saved?.public_key ?? '', secret_key: '', mode: saved?.mode ?? saved?.settings?.mode ?? 'sandbox', enabled: saved?.enabled ?? saved?.is_connected ?? true });
    ...
    const saveAccount = async (event) => {
        ...
        try {
            const updated = await apiRequest('put', '/provider/payment-accounts', { gateway: activeGateway.id, ...account, settings: { ...(account.secret_key ? { secret_key: account.secret_key } : {}), ...(activeGateway.id === 'paypal' ? { mode: account.mode } : {}) } });
    ...
    return <div className="space-y-6">
        ...
        {activeGateway && <div className="fixed inset-0 z-[70] grid place-items-end bg-slate-950/35 backdrop-blur-sm sm:place-items-center sm:p-4" onMouseDown={() => setActiveGateway(null)}><Card className="w-full max-w-lg rounded-b-none sm:rounded-3xl" onMouseDown={(event) => event.stopPropagation()}><h2 className="text-lg font-bold text-slate-950">Connect {activeGateway.name}</h2><p className="mt-1 text-sm text-slate-500">Use the credentials from your own {activeGateway.name} dashboard. Booking payments will be initialized and verified on that provider account.</p><form className="mt-5 space-y-4" onSubmit={saveAccount}>{activeGateway.id === 'paypal' && <Field label="PayPal mode"><select className={inputClass} onChange={(event) => setAccount((current) => ({ ...current, mode: event.target.value }))} value={account.mode}><option value="sandbox">Sandbox</option><option value="live">Live</option></select></Field>}<Field label={activeGateway.id === 'paypal' ? 'PayPal client ID' : `${activeGateway.name} public key`}><input className={inputClass} onChange={(event) => setAccount((current) => ({ ...current, public_key: event.target.value }))} placeholder={activeGateway.id === 'paypal' ? 'PayPal REST app client ID' : 'pk_test_...'} required value={account.public_key} /></Field><Field hint="Encrypted after saving. Leave blank when editing to keep the saved key." label={activeGateway.id === 'paypal' ? 'PayPal client secret' : `${activeGateway.name} secret key`}><input className={inputClass} onChange={(event) => setAccount((current) => ({ ...current, secret_key: event.target.value }))} placeholder={(Array.isArray(accounts) ? accounts.find((item) => item.gateway === activeGateway.id) : accounts?.[activeGateway.id])?.has_secret_key ? 'Saved — leave blank to keep current key' : activeGateway.id === 'paypal' ? 'PayPal REST app secret' : 'sk_test_...'} type="password" value={account.secret_key} /></Field><label className="flex items-center gap-3 text-sm font-semibold text-slate-700"><input checked={account.enabled} className="size-4 accent-fuchsia-600" onChange={(event) => setAccount((current) => ({ ...current, enabled: event.target.checked }))} type="checkbox" />Accept booking payments through this gateway</label><div className="flex justify-end gap-2"><Button onClick={() => setActiveGateway(null)} type="button" variant="secondary">Cancel</Button><Button busy={saving} type="submit">Save connection</Button></div></form></Card></div>}
    </div>;
}
