...
function OpportunityCard({ item, index }) {
    ...
    return (
        <article className="group grid gap-5 rounded-[1.7rem] border border-[#BFC3C8] bg-white p-5 shadow-[0_16px_45px_rgba(52,35,28,.06)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_26px_70px_rgba(52,35,28,.11)] lg:grid-cols-[86px_1fr_auto] lg:items-center">
            <div className="flex items-center gap-4 lg:block">
                <span className="grid size-16 place-items-center rounded-2xl bg-[#F7F3ED] font-display text-2xl font-normal text-[#3A2A1F]">{String(index + 1).padStart(2, '0')}</span>
                <span className="rounded-full bg-[#F7F3ED] px-3 py-1 text-[10px] font-semibold uppercase tracking-wide text-[#3A2A1F] lg:hidden">{labelFor(item.type)}</span>
            </div>
            <div className="min-w-0">
                <div className="hidden flex-wrap items-center gap-3 lg:flex">
                    <span className="inline-flex items-center gap-1.5 rounded-full bg-[#F7F3ED] px-3 py-1 text-[10px] font-semibold uppercase tracking-wide text-[#3A2A1F]">
                    ...
                    {item.deadline && <span className={`text-xs font-semibold ${urgent ? 'text-[#3A2A1F]' : 'text-stone-400'}`}>Closes {shortDate(item.deadline)}</span>}
                </div>
                <h2 className="mt-2 font-display text-2xl font-normal leading-tight text-[#2A1D14] sm:text-3xl">{item.title ?? labelFor(item.type)}</h2>
                <p className="mt-3 line-clamp-2 text-sm leading-6 text-[#3A2A1F]">{cardDescription}</p>
            </div>
            <Link to={`/opportunities/${item.id}`} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full bg-[#2A1D14] px-6 text-sm font-semibold text-white transition hover:bg-[#2A1D14] lg:justify-self-end">
...
function TypeTile({ type, active, onClick }) {
    return (
        <button type="button" onClick={onClick} className={`group rounded-[1.5rem] border p-5 text-left transition duration-300 ${active ? 'border-[#2A1D14] bg-[#2A1D14] text-white shadow-[0_22px_60px_rgba(52,35,28,.18)]' : 'border-[#BFC3C8] bg-white/80 text-[#2A1D14] hover:-translate-y-1 hover:bg-white hover:shadow-[0_18px_45px_rgba(52,35,28,.08)]'}`}>
            <span className={`grid size-11 place-items-center rounded-2xl ${active ? 'bg-white text-[#2A1D14]' : 'bg-[#F7F3ED] text-[#3A2A1F]'}`}>
                <Icon name={typeIcons[type] ?? 'external'} size={18} />
            </span>
            <p className="mt-5 text-xs font-semibold uppercase tracking-wide">{labelFor(type)}</p>
            <p className={`mt-2 text-sm font-semibold ${active ? 'text-white/70' : 'text-[#3A2A1F]'}`}>View category</p>
        </button>
    );
}
...
export default function OpportunitiesPage() {
    ...
    return (
        <>
            ...
            />
            <section className="relative isolate overflow-hidden bg-[#F7F3ED] py-12 sm:py-16">
                ...
                <div className="page-container">
                    <div>
                        <h1 className="max-w-5xl font-display text-5xl font-normal leading-[.94] text-[#2A1D14] sm:text-7xl">Find collaborations, features, and industry doors worth opening.</h1>
                        <p className="mt-6 max-w-2xl text-base leading-8 text-[#3A2A1F]">Browse approved opportunities submitted through BPHQ, from brand collaborations to speaking slots and media requests.</p>
                        <div className="mt-8 flex flex-wrap gap-3">
                            <Button onClick={() => chooseType('all')} className="rounded-full bg-[#2A1D14] px-7 hover:bg-[#2A1D14]">
                                Browse opportunities <Icon name="arrow" size={15} />
                            </Button>
                            <Link to="/opportunities#opportunity-list" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full border border-[#BFC3C8] bg-white/75 px-6 text-sm font-bold text-[#2A1D14] transition hover:bg-white">
            ...
            <section id="opportunity-list" className="bg-white py-12 sm:py-16">
                <div className="page-container">
                    <div className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                        <div>
                            <p className="text-xs font-semibold uppercase tracking-[.18em] text-[#3A2A1F]">Browse by type</p>
                            <h2 className="mt-2 font-display text-4xl font-normal text-[#2A1D14]">Choose the right fit.</h2>
                        </div>
            ...
            <section className="bg-[#F7F3ED] py-14">
                <div className="page-container">
                    <div className="rounded-[2rem] bg-[#2A1D14] p-7 text-white shadow-[0_26px_80px_rgba(52,35,28,.18)] sm:p-10 lg:flex lg:items-center lg:justify-between lg:gap-8">
                        <div>
                            <p className="text-xs font-semibold uppercase tracking-[.2em] text-rose-200">Submit an opportunity</p>
