                                    ...
                                    {approvalProviders.map((provider) => {
                                        ...
                                        return (
                                            <tr className="border-b border-slate-50 last:border-0" key={provider.id}>
                                                ...
                                                <td className="py-3"><div className="flex justify-end gap-2"><Button busy={isBusy(`approve-access-${provider.id}`)} onClick={() => approveAccess(provider)} type="button">Approve</Button><Button busy={isBusy(`decline-access-${provider.id}`)} onClick={() => declineAccess(provider)} type="button" variant="danger">Decline</Button>{provider.slug && <Link to={`/admin/providers/${provider.slug}/preview`} target="_blank" className="inline-flex min-h-10 items-center justify-center rounded-xl border border-slate-200 bg-white px-4 text-sm font-bold text-slate-700 transition hover:bg-slate-50">View</Link>}<Button onClick={() => startEdit(provider)} type="button" variant="secondary">Edit</Button></div></td>
                                            </tr>
                                        );
                                    })}
                                </tbody>
