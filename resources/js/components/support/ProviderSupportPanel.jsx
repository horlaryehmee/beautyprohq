        ...
        <div className="space-y-5">
            <Card className="overflow-hidden border-bphq-chrome/70 bg-gradient-to-br from-bphq-ivory via-white to-fuchsia-50" padding={false}>
                <div className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6">
                    ...
                    <Button onClick={() => { setSelected(null); setRequestModalOpen(true); }} type="button">New support request</Button>
                </div>
            </Card>
                ...
                <div className="grid min-h-[620px] lg:grid-cols-[330px_minmax(0,1fr)]">
                    <aside className={`${selected ? 'hidden lg:block' : 'block'} border-b border-slate-100 lg:border-b-0 lg:border-r`}>
                        ...
                        <div className="max-h-[540px] overflow-y-auto">
                            ...
                            }) : <EmptyState action={<Button onClick={() => setRequestModalOpen(true)} type="button" variant="soft">Start a request</Button>} description="When you need our team, your requests will live here." icon="chat" title="No support requests" />}
                        </div>
                    </aside>

                    <section className={`${selected ? 'flex' : 'hidden lg:flex'} min-h-0 flex-col bg-slate-50`}>
                        {selected ? <>
                            <header className="flex items-center justify-between gap-3 border-b border-slate-100 bg-white p-4"><div className="flex min-w-0 items-center gap-3"><Button className="lg:hidden" onClick={() => setSelected(null)} type="button" variant="secondary">Back</Button><div className="min-w-0"><div className="flex items-center gap-2"><h3 className="truncate text-base font-bold text-slate-950">{selected.subject}</h3><StatusBadge status={selected.status} /></div><p className="mt-1 text-xs text-slate-500">Request #{selected.id} · {selected.category.replace('_', ' ')}</p></div></div><Button onClick={() => setRequestModalOpen(true)} type="button" variant="secondary">New</Button></header>
            ...
            {requestModalOpen && <div className="fixed inset-0 z-[70] grid place-items-end bg-slate-950/35 p-0 backdrop-blur-sm sm:place-items-center sm:p-4" onMouseDown={() => !creatingRequest && setRequestModalOpen(false)}><Card className="w-full max-w-xl rounded-b-none sm:rounded-3xl" onMouseDown={(event) => event.stopPropagation()}><CardHeader description="Share enough detail for us to investigate quickly. Never include passwords, card details, or API keys." title="New support request" /><form className="space-y-4" onSubmit={createTicket}><Field label="What do you need help with?"><input className={inputClass} maxLength="180" onChange={(event) => setForm((current) => ({ ...current, subject: event.target.value }))} placeholder="For example: I need help with my provider profile" required value={form.subject} /></Field><Field label="Category"><select className={inputClass} onChange={(event) => setForm((current) => ({ ...current, category: event.target.value }))} value={form.category}>{categories.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></Field><Field label="Details"><textarea className={`${inputClass} min-h-36`} maxLength="6000" minLength="10" onChange={(event) => setForm((current) => ({ ...current, message: event.target.value }))} placeholder="Tell us what happened, what you expected, and any useful details." required value={form.message} /></Field><div className="flex justify-end gap-2"><Button disabled={creatingRequest} onClick={() => setRequestModalOpen(false)} type="button" variant="secondary">Cancel</Button><Button busy={creatingRequest} type="submit">Send request</Button></div></form></Card></div>}
        </div>
    );
}
