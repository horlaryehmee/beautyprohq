...
<head>
    ...
    <style>
        ...
        *{box-sizing:border-box}html{min-height:100%;background:#fdfbf8;color:#29131f;font-family:Inter,'Avenir Next','Segoe UI',system-ui,sans-serif}body{min-height:100vh;margin:0;overflow-x:hidden}
        ...
        .wrap{display:grid;min-height:100vh;width:min(920px,calc(100% - 32px));margin:0 auto;padding:18px 0;grid-template-rows:auto auto auto;align-content:center;justify-items:center;gap:16px}
        ...
        main{display:grid;width:100%;justify-items:center;gap:16px;padding:6px 0 8px;overflow:hidden}
        .image-marquees{display:grid;width:min(100vw,920px);gap:12px;overflow:hidden;padding:2px 0;pointer-events:none}.image-row{display:flex;width:max-content;gap:12px;animation:image-marquee-left 34s linear infinite}.image-row.reverse{animation-name:image-marquee-right}.image-row img{width:150px;height:86px;border:1px solid rgba(255,255,255,.86);border-radius:10px;object-fit:cover;box-shadow:0 14px 35px rgba(41,19,31,.16)}
        .copy{width:min(720px,100%);text-align:center}.copy:after{display:block;width:86px;height:3px;margin:14px auto 0;border-radius:999px;background:linear-gradient(90deg,#54213f,#c93661,#15816f);content:""}
        h1{margin:0;color:#29131f;font-family:'Playfair Display',Georgia,serif;font-size:clamp(38px,6.4vw,74px);font-weight:400;line-height:.96;letter-spacing:0}.lead{max-width:620px;margin:12px auto 0;color:#51443e;font-size:clamp(15px,1.55vw,18px);line-height:1.45}
        .panel{position:relative;width:min(460px,100%);overflow:hidden;border:1px solid rgba(80,44,66,.16);border-radius:10px;background:linear-gradient(145deg,rgba(255,255,255,.93),rgba(253,251,248,.82));box-shadow:0 24px 70px rgba(41,19,31,.16);backdrop-filter:blur(20px);padding:17px;transition:transform .16s ease}.panel:before{content:"";position:absolute;inset:0 0 auto;height:3px;background:linear-gradient(90deg,#54213f,#c93661,#15816f);background-size:200% 100%;animation:sweep 3.5s linear infinite}.panel h2{margin:0;color:#29131f;font-family:'Playfair Display',Georgia,serif;font-size:25px;font-weight:400;line-height:1.05}.panel p{margin:6px 0 0;color:#6b5b53;font-size:14px;line-height:1.38}
        ...
        footer{display:flex;flex-wrap:wrap;justify-content:center;gap:12px;color:#78716c;font-size:12px}.links{display:flex;gap:12px}.links a{color:#502c42;text-decoration:none;font-weight:850}
        ...
        @media (max-width:560px){.wrap{width:min(100% - 20px,920px);padding:14px 0;gap:13px}.brand-bphq{font-size:1.55rem}.brand-name{font-size:7px;letter-spacing:.3em}main{gap:12px;padding:0}.copy:after{width:68px;margin-top:10px}.image-marquees{width:calc(100vw - 20px);gap:8px}.image-row{gap:8px;animation-duration:26s}.image-row img{width:112px;height:70px;border-radius:8px}h1{font-size:32px;line-height:.98}.lead{margin-top:9px;font-size:14px;line-height:1.34}.panel{padding:13px}.panel h2{font-size:22px}.panel p{font-size:13px;line-height:1.3}form{grid-template-columns:1fr;gap:8px;margin-top:10px}button{width:100%;min-height:40px}.marquee{padding:6px 0;font-size:9px;letter-spacing:.1em}footer{font-size:11px}}
        @media (max-height:700px) and (max-width:560px){.wrap{gap:10px}.lead,.marquee,footer{display:none}.panel p{display:none}}
...
<body>
    <div class="page">
        ...
        <div class="wrap">
            <header>
                <a class="brand" href="/" aria-label="BeautyPro HQ">
                    <span class="brand-bphq">BPHQ</span>
                    <span class="brand-name">BEAUTYPROHQ</span>
                </a>
            </header>
            <main>
            ...
            <div class="marquee" aria-label="BeautyPro HQ launch areas">
                <div class="track">
                    <span>Beauty bookings</span><span>Trusted professionals</span><span>Provider tools</span><span>Events and opportunities</span>
                    <span>Beauty bookings</span><span>Trusted professionals</span><span>Provider tools</span><span>Events and opportunities</span>
                </div>
            </div>
            <footer>
                <span>&copy; {{ date('Y') }} BeautyPro HQ. All rights reserved.</span>
                <span class="links"><a href="mailto:hello@beautyprohq.com">Contact</a></span>
            </footer>
        </div>
    </div>
    <script>
        ...
        form.addEventListener('submit', async (event) => {
            ...
            try {
                ...
                if (!response.ok) throw new Error(data.message || 'Please check the email address and try again.');
                form.reset();
                message.textContent = data.message || 'You are on the list. We will send launch updates soon.';
                message.className = 'message ok show';
            } catch (error) {
                message.textContent = error.message || 'Something went wrong. Please try again.';
                message.className = 'message err show';
            } finally {
