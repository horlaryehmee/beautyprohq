...
<head>
    ...
    <style>
        ...
        *{box-sizing:border-box}html{min-height:100%;background:#fdfbf8;color:#29131f;font-family:Inter,'Avenir Next','Segoe UI',system-ui,sans-serif}body{min-height:100vh;margin:0;overflow-x:hidden}
        ...
        .wrap{display:grid;min-height:100vh;width:min(1040px,calc(100% - 32px));margin:0 auto;padding:18px 0;grid-template-rows:auto auto auto;align-content:center;justify-items:center;gap:18px}
        ...
        main{position:relative;display:grid;grid-template-columns:minmax(0,1fr) minmax(300px,.62fr);gap:28px;align-items:center;padding:12px 0}
        .image-marquees{position:absolute;z-index:0;inset:37% 0 auto 0;display:grid;gap:14px;overflow:hidden;padding:2px 0;pointer-events:none}.image-row{display:flex;width:max-content;gap:14px;animation:image-marquee-left 34s linear infinite}.image-row.reverse{animation-name:image-marquee-right}.image-row img{width:156px;height:88px;border:1px solid rgba(255,255,255,.86);border-radius:10px;object-fit:cover;box-shadow:0 14px 35px rgba(41,19,31,.16)}
        ...
        @media (max-width:900px){.page{background-attachment:scroll}main{display:flex;width:100%;max-width:620px;flex-direction:column;gap:16px;padding:0}.copy{order:1;width:100%;padding-left:0;text-align:center}.copy:before{display:none}.image-marquees{position:relative;inset:auto;order:2;width:calc(100% + 32px);margin:0 -16px;gap:10px}.panel{order:3;width:100%;max-width:560px}.image-row{gap:10px;animation-duration:30s}.image-row img{width:138px;height:82px}}
        @media (max-width:560px){.wrap{width:min(100% - 20px,1040px);padding:14px 0;gap:14px}.brand-bphq{font-size:1.55rem}.brand-name{font-size:7px;letter-spacing:.3em}main{gap:12px}.image-marquees{width:calc(100% + 20px);margin:0 -10px;gap:8px}.image-row{gap:8px;animation-duration:26s}.image-row img{width:116px;height:72px;border-radius:8px}h1{font-size:34px;line-height:.98}.lead{margin-top:9px;font-size:14px;line-height:1.34}.panel{padding:13px}.panel h2{font-size:22px}.panel p{font-size:13px;line-height:1.3}form{grid-template-columns:1fr;gap:8px;margin-top:10px}button{width:100%;min-height:40px}.marquee{padding:6px 0;font-size:9px;letter-spacing:.1em}footer{font-size:11px}}
