...
<div class="min-h-screen bg-cream-50 text-plum-950" data-bphq-first-paint>
    <header class="sticky top-0 z-50 overflow-hidden border-b border-stone-200/70 bg-[#F7F3ED]/96 shadow-[0_8px_28px_rgba(52,35,28,.06)] backdrop-blur-xl lg:hidden" style="height: {{ $brandMobileHeaderHeight ?? 96 }}px; --bphq-mobile-header-height: {{ $brandMobileHeaderHeight ?? 96 }}px; --bphq-mobile-logo-height: {{ max(28, ($brandMobileHeaderHeight ?? 96) - 8) }}px; --bphq-mobile-control-size: {{ max(32, min(40, ($brandMobileHeaderHeight ?? 96) - 16)) }}px;">
        <div class="flex h-full items-center justify-between overflow-hidden px-4">
            ...
            <a href="/" class="inline-flex h-full max-w-[58vw] items-center overflow-hidden leading-none" aria-label="BeautyPro HQ home">
                <img src="/brand/bphq-logo-transparent.svg" alt="BeautyPro HQ" width="168" height="168" class="block max-h-full max-w-full w-auto object-contain" style="height: var(--bphq-mobile-logo-height);">
            </a>
            <a href="/login" class="grid place-items-center rounded-2xl border border-stone-200 bg-white text-[#2A1D14]" style="height: var(--bphq-mobile-control-size); width: var(--bphq-mobile-control-size);" aria-label="Account">
    ...
    <main class="pb-20 lg:pb-0">
        <section class="bg-[#F7F3ED] text-[#2A1D14]">
            <div class="page-container grid grid-cols-1 items-center gap-7 pb-8 pt-12 sm:pt-16 md:min-h-[520px] md:grid-cols-[.94fr_1.06fr] md:gap-8 md:pb-10 md:pt-20 lg:gap-12 lg:pb-14 lg:pt-24">
                <div class="hero-copy-enter">
                    <h1 class="mx-auto mt-2 max-w-[620px] text-center font-display text-[3rem] font-normal leading-[.9] text-[#2A1D14] sm:text-[3.7rem] md:mx-0 md:mt-0 md:text-left md:text-[clamp(2.75rem,6.5vw,5.2rem)]">
                        <span>The Beauty Service </span>
                        <span class="block font-display italic text-[#3A2A1F]">Ecosystem</span>
                    </h1>
                    ...
                    <p class="mx-auto mt-3 max-w-xl text-center text-sm font-normal leading-6 text-[#3A2A1F] md:mx-0 md:text-left md:text-lg md:leading-7">
                    ...
                    <div class="mt-6 flex flex-wrap justify-center gap-3 md:justify-start">
                        <a href="/register" class="inline-flex min-h-12 items-center justify-center gap-2 rounded-md bg-[#2A1D14] px-7 text-xs font-semibold uppercase tracking-wide text-white transition hover:bg-[#3A2A1F] active:scale-95">
                            Join BPHQ
                        ...
                        <a href="/directory" class="inline-flex min-h-12 items-center justify-center rounded-md border border-[#BFC3C8] bg-[#F7F3ED] px-7 text-xs font-semibold uppercase tracking-wide text-[#2A1D14] transition hover:bg-white active:scale-95">Explore Directory</a>
                    </div>
                        ...
                        <div class="flex w-max items-center whitespace-nowrap">
                            @foreach([[500, 'Beauty Pros'], [50, 'Cities'], [100, 'Resources'], [25, 'Events']] as [$value, $label])
                                <div class="flex items-center">
                                    @if(!$loop->first)<span class="mx-5 h-8 w-px bg-gradient-to-b from-transparent via-[#DCCCB8] to-transparent"></span>@endif
                                    <span class="inline-flex flex-col gap-1">
                                        <span class="font-display text-2xl font-semibold leading-none text-[#2A1D14]">
                                        ...
                                        <span class="text-[10px] font-semibold uppercase tracking-[.15em] text-[#3A2A1F]">{{ $label }}</span>
                                    </span>
                                </div>
                            @endforeach
                        </div>
                ...
                <div class="mx-auto w-full md:max-w-none">
                    <div class="relative mx-auto h-[360px] w-full max-w-[500px] overflow-hidden rounded-[1.6rem] sm:h-[430px] md:h-[540px] md:rounded-[2rem] lg:h-[620px]">
                        <div class="pointer-events-none absolute inset-x-0 top-0 z-10 h-24 bg-gradient-to-b from-[#F7F3ED] to-transparent"></div>
                        <div class="pointer-events-none absolute inset-x-0 bottom-0 z-10 h-24 bg-gradient-to-t from-[#F7F3ED] to-transparent"></div>
    ...
    <nav aria-label="Mobile navigation" class="fixed inset-x-3 bottom-[max(.75rem,env(safe-area-inset-bottom))] z-[80] translate-y-0 opacity-100 transition-all duration-300 ease-out lg:hidden">
        <div class="mx-auto flex w-fit max-w-full items-center justify-center gap-1 rounded-2xl border border-stone-200/90 bg-white/95 p-1.5 shadow-[0_12px_40px_rgba(41,19,31,.18)] backdrop-blur-xl">
            <a href="/" aria-current="page" aria-label="Home" class="relative flex h-11 min-w-10 items-center justify-center gap-1.5 rounded-xl bg-rose-50 px-3 text-xs font-semibold text-rose-600">
            ...
            <a href="/news" aria-label="News" class="relative flex h-11 min-w-10 items-center justify-center gap-0 rounded-xl px-[.55rem] text-xs font-semibold text-stone-500">
            ...
            <a href="/events" aria-label="Events" class="relative flex h-11 min-w-10 items-center justify-center gap-0 rounded-xl px-[.55rem] text-xs font-semibold text-stone-500">
            ...
            <a href="/directory" aria-label="Directory" class="relative flex h-11 min-w-10 items-center justify-center gap-0 rounded-xl px-[.55rem] text-xs font-semibold text-stone-500">
