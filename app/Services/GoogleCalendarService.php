...
class GoogleCalendarService
{
    ...
    private const SCOPE = 'openid email https://www.googleapis.com/auth/calendar.app.created';
    ...
    public function createBookingCalendar(string $accessToken, string $timezone): string
    {
        $response = $this->calendarRequest($accessToken)->post(self::CALENDAR_API_URL.'/calendars', [
            'summary' => 'BeautyPro HQ Bookings',
            'description' => 'Bookings created automatically by BeautyPro HQ.',
            'timeZone' => $timezone,
        ])->throw();

        $calendarId = $response->json('id');
        if (blank($calendarId)) {
            throw new \RuntimeException('Google did not return the new BeautyPro HQ calendar ID.');
        }
    ...
    private function syncBooking(Booking $booking): bool
    {
        ...
        $booking->loadMissing(['provider.user', 'provider.calendarConnection', 'customer', 'service']);
    ...
    private function eventPayload(Booking $booking): array
    {
        ...
        $start = CarbonImmutable::createFromFormat('Y-m-d H:i:s', $date.' '.$this->clock($booking->time), $timezone);
